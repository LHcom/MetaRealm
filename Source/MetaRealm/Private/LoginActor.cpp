...
void ALoginActor::ShowErrMsg(bool bUiType , FString& MSG)
{
	if ( bUiType )
	{
		// 회원가입
		SignUpUI->ShowErrMsg(MSG);
	}
	else
	{
		// 로그인
		LoginUI->ShowErrMsg(MSG);
	}
}

void ALoginActor::HideSignUpUI()
{
	if ( !SignUpUI )
		return;

	SignUpUI->ExitButtonClicked();
}
